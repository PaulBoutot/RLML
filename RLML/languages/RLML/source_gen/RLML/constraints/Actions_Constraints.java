...
public class Actions_Constraints extends BaseConstraintsDescriptor {
  ...
  public static class Value_Property extends BasePropertyConstraintsDescriptor {
    public Value_Property(ConstraintsDescriptor container) {
      super(PROPS.value$MI6P, container, false, false, true);
    }
    @Override
    public boolean validateValue(SNode node, Object propertyValue, CheckingNodeContext checkingNodeContext) {
      boolean result = staticValidateProperty(node, SPropertyOperations.castString(propertyValue));
      if (!(result) && checkingNodeContext != null) {
        checkingNodeContext.setBreakingNode(new SNodePointer("r:12ad0550-f678-466c-bace-38d01386f6fd(RLML.constraints)", "5647291236746308290"));
      }
      return result;
    }
    private static boolean staticValidateProperty(SNode node, String propertyValue) {
      ...
      ArrayList<ArrayList<Integer>> actionsArrLst = new ArrayList<ArrayList<Integer>>();
      ...
      for (String arr : strArr) {
        ...
        actionsArrLst.add(arrArrInt);
      }

      // Convert from ArrayList to int[][]
      int[][] actions = new int[actionsArrLst.size()][];
      for (int i = 0; i < actions.length; i++) {
        actions[i] = new int[actionsArrLst.get(i).size()];
      }
      for (int i = 0; i < actionsArrLst.size(); i++) {
        for (int j = 0; j < actionsArrLst.get(i).size(); j++) {
          actions[i][j] = actionsArrLst.get(i).get(j);
        }
      }
  ...
  protected Map<SProperty, PropertyConstraintsDescriptor> getSpecifiedProperties() {
    ...
    properties.put(PROPS.value$MI6P, new Value_Property(this));
    return properties;
  }

  private static final class CONCEPTS {
    /*package*/ static final SConcept Actions$Mg = MetaAdapterFactory.getConcept(0x3c2f74fb565a4cb8L, 0x8a8142024cc7aa10L, 0x1d76fb9dad847c98L, "RLML.structure.Actions");
